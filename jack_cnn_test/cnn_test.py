...
def raw_stroke_to_normed(stroke:np.ndarray,max_stroke_len:int):
    ...
    padded_stroke = np.pad(normed_arr,((0,pad_len),(0,0)),constant_values=-1)
...
class StrokeModel(nn.Module):
    def __init__(self, base_c=64, depth=3, hidden_size=512, dropout_p=0.2, max_stroke_len=100, img_length=256,
        ...
        bottleneck_c = self.c_levels[-1]
        self.bottleneck_conv = ConvBlock(bottleneck_c + self.embed_dim, bottleneck_c, dropout_rate=dropout_p + 0.1)
        ...
        self.param_head = nn.Sequential(
            nn.Flatten(),
            nn.Linear(bottleneck_c * final_h_w * final_h_w, hidden_size),
            nn.ReLU(inplace=True),
        ...
        self.h_init = nn.Linear(bottleneck_c * final_h_w * final_h_w, hidden_size)
        self.c_init = nn.Linear(bottleneck_c * final_h_w * final_h_w, hidden_size)

        # Attention Mechanism (Context from bottleneck, Hidden from LSTM)
        self.attention = Attention(encoder_dim=bottleneck_c, decoder_dim=hidden_size)

        # LSTM Decoder (Input: previous coordinate (2) + context vector (hidden_size))
        self.lstm = nn.LSTM(input_size=2 + bottleneck_c,
    ...
    def forward(self, img_x, text_input):
        ...
        context = self.bottleneck_conv(x)
...
# 1. Sequence (Coordinate) Loss and EOS Loss
class SequenceLoss(nn.Module):
    ...
    def forward(self, pred_coords, pred_eos_logits, target_coords, target_eos):
        ...
        valid_length_mask = (target_coords.sum(dim=2) != 0).float()
...
# 3. Parameter Loss (MSE for all 5 parameters)
class ParameterLoss(nn.Module):
    ...
    def forward(self, pred_params, target_params):
        # pred_params: [B, 5], target_params: [B, 5]
...
lambda_seq_eos = 0.5    # Weight for EOS Loss (BCE)
...
model = StrokeModel(input_size=input_img_size, depth=model_depth, embed_dim=768).to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)

# --- LOSS FUNCTIONS ---
criterion_sequence = SequenceLoss()
criterion_params = ParameterLoss()
...
for epoch in range(num_epochs):
    ...
    # Example: total_steps = len(train_dataloader)
    total_steps = 200 # Placeholder for demonstration
    for i in range(total_steps):
        # 1. Load and move data (SIMULATION)
        # img_x, bert_embeds, target_params, target_coords, target_eos = next(iter(train_dataloader))

        # --- SIMULATED DATA (Replace with actual data) ---
        B, L = 16, 50
        img_x = torch.randn(B, 1, input_img_size, input_img_size).to(device)
        bert_embeds = torch.randn(B, 768).to(device)
        target_params = torch.randn(B, 5).to(device)
        target_coords = torch.randn(B, L, 2).to(device)
        # Simple EOS: set to 1.0 at the last step for demonstration
        target_eos = (torch.arange(L).unsqueeze(0).to(device) == (L-1)).float().repeat(B, 1)
        # --- END SIMULATED DATA ---
        ...
        pred_params, h0, c0, context_features = model(img_x, bert_embeds)
    ...
    model.eval()
    val_loss = 0.0
    val_steps = 50 # Placeholder for demonstration
    with torch.no_grad():
